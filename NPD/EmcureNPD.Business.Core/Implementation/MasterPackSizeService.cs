...
namespace EmcureNPD.Business.Core.Implementation
{
    public class MasterPackSizeService : IMasterPackSizeService
    {
        ...
        public async Task<DBOperation> AddUpdatePackSize(MasterPackSizeEntity entityPackSize)
        {
            ...
            {
                var objModelData = _repository.Exists(x => x.PackSizeName.ToLower() == entityPackSize.PackSizeName.ToLower() && x.PackSizeId!= entityPackSize.PackSizeId);
                if (objModelData)
            ...
            {
                var objModelData = _repository.Exists(x => x.PackSizeName.ToLower() == entityPackSize.PackSizeName.ToLower());
                if (objModelData)
                { return DBOperation.AlreadyExist; }
                objPackSize = _mapperFactory.Get<MasterPackSizeEntity, MasterPackSize>(entityPackSize);
