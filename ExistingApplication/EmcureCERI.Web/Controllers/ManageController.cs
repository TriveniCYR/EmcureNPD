...
using EmcureCERI.Web.Models.ManageViewModels;
...
namespace EmcureCERI.Web.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            var aspuser = _db.AspNetUsers.Where(o => o.Email == user.Email).FirstOrDefault();
            aspuser.IsChangePassword = true;
            _db.SaveChanges();
        ...
        public IActionResult SecurityQuestion()
        {
            ...
            SQViewModel model = new SQViewModel { StatusMessage = StatusMessage };
            ...
            for (int i = 0; i < model22.Count(); i++)
            {
                ...
                {
                    case 0:
                        ViewBag.SelectedQ1 = model22[i].QuesId;
                        model.Answer1 = model22[i].Answer;
                        break;
                    case 1:
                        ViewBag.SelectedQ2 = model22[i].QuesId;
                        model.Answer2 = model22[i].Answer;
                        break;
                    case 2:
                        ViewBag.SelectedQ3 = model22[i].QuesId;
                        model.Answer3 = model22[i].Answer;
                        break;
                }
            }
            return View(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult SecurityQuestion(SQViewModel model)
        {
            if (ModelState.IsValid)
            {
                int userId = Convert.ToInt32(HttpContext.Session.GetString("CurrentUserId"));
                var cultureName = System.Globalization.CultureInfo.CurrentCulture.Name;
                List<QuestionAnswer> qaModel = _qa.GetAllQuesAnsByUserId(userId, cultureName);

                for (int i = 0; i < qaModel.Count(); i++)
                {
                    switch (i)
                    {
                        case 0:
                            AnswerDetails ans1 = _qa.GetAnswer(qaModel[i].Id);
                            ans1.QuestId = Convert.ToInt32(model.Question1);
                            ans1.Answer = model.Answer1;
                            _qa.UpdateAnswerMap(ans1);
                            break;
                        case 1:
                            AnswerDetails ans2 = _qa.GetAnswer(qaModel[i].Id);
                            ans2.QuestId = Convert.ToInt32(model.Question2);
                            ans2.Answer = model.Answer2;
                            _qa.UpdateAnswerMap(ans2);
                            break;
                        case 2:
                            AnswerDetails ans3 = _qa.GetAnswer(qaModel[i].Id);
                            ans3.QuestId = Convert.ToInt32(model.Question3);
                            ans3.Answer = model.Answer3;
                            _qa.UpdateAnswerMap(ans3);
                            break;
                    }
                }

                StatusMessage = "Your Security Question has been changed.";
                return RedirectToAction(nameof(SecurityQuestion));
            }
            return View(model);
        }
        ...
        private string GenerateQrCodeUri(string email, string unformattedKey)
        {
            return string.Format(
                ...
                _urlEncoder.Encode("EmcureCERI.Web"),
